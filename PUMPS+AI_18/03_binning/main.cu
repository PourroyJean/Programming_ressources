...
//@@ INSERT CODE HERE
    ...
    if (outIdx < grid_size) {
        ...
        const int MY_binIdx = (int) ((outIdx / grid_size) * NUM_BINS);  //In which bin Am I ?

        //TO THE LEFT: until 0 or too far
        int MY_inIdx = bin_ptrs[MY_binIdx];
        for (int inIdx = MY_inIdx; inIdx >= 0; --inIdx) {
        ...
        //TO the right: until the end or too far
        MY_inIdx++;
        for (int inIdx = MY_inIdx; inIdx < bin_ptrs[NUM_BINS]; ++inIdx) {
            const float dist = in_pos_sorted[inIdx] - (float) outIdx;
            if (dist < cutoff2) {
                const float in_val2 = in_val_sorted[inIdx] * in_val_sorted[inIdx];
                out[outIdx] += in_val2 / (dist * dist);
...
// !!!!!!!!
// !!!!!!!!
        //The following code works if there is only 1 bin on each side to visit
//        int inIdx_start = -1;
//        int inIdx_end = -1;
        //start
//        if (MY_binIdx == 0) {
//            inIdx_start = 0;
//        } else {
//            inIdx_start = bin_ptrs[MY_binIdx - 1]; //-2
//        }
//
//        //end
//        if (MY_binIdx == NUM_BINS - 1) {
//            inIdx_end = bin_ptrs[NUM_BINS];
//        } else {
//            inIdx_end = bin_ptrs[MY_binIdx + 2]; //+1
//        }
//        for (int inIdx = inIdx_start; inIdx < inIdx_end; ++inIdx) {
//            const float dist = in_pos_sorted[inIdx] - (float) outIdx;
//            if (dist < cutoff2) {
//                const float in_val2 = in_val_sorted[inIdx] * in_val_sorted[inIdx];
//                out[outIdx] += in_val2 / (dist * dist);
...
int eval(const int num_in, const int max, const int grid_size) {
    ...
    const std::string conf_info =
            std::string("[len:") + std::to_string(num_in) + "/max:" + std::to_string(max) + "/gridSize:" +
    ...
    if (mode == Mode::GPUBinnedCPUPreprocessing) {
        ...
        cpu_preprocess(in_val_h.data(), in_pos_h.data(), in_val_sorted_h.data(), in_pos_sorted_h.data(),
                       grid_size,
                       num_in, bin_counts_h.data(),
                       bin_ptrs_h.data());
        timer_stop();
    }
    ...
    if (mode != Mode::CPUNormal) {
        ...
        if (mode != Mode::GPUBinnedCPUPreprocessing) {
            THROW_IF_ERROR(
                    cudaMemcpy(in_val_d, in_val_h.data(), num_in * sizeof(float), cudaMemcpyHostToDevice));
            THROW_IF_ERROR(
                    cudaMemcpy(in_pos_d, in_pos_h.data(), num_in * sizeof(float), cudaMemcpyHostToDevice));
        }
        ...
        if (mode == Mode::GPUBinnedCPUPreprocessing) {
            THROW_IF_ERROR(cudaMemcpy(in_val_sorted_d, in_val_sorted_h.data(), num_in * sizeof(float),
                                      cudaMemcpyHostToDevice));
            THROW_IF_ERROR(cudaMemcpy(in_pos_sorted_d, in_pos_sorted_h.data(), num_in * sizeof(float),
                                      cudaMemcpyHostToDevice));
            THROW_IF_ERROR(
                    cudaMemcpy(bin_ptrs_d, bin_ptrs_h.data(), (NUM_BINS + 1) * sizeof(int),
                               cudaMemcpyHostToDevice));
    ...
    if (mode == Mode::GPUBinnedGPUPreprocessing) {
        ...
        gpu_preprocess(in_val_d, in_pos_d, in_val_sorted_d, in_pos_sorted_d, grid_size, num_in, bin_counts_d,
                       bin_ptrs_d);
        THROW_IF_ERROR(cudaDeviceSynchronize());
        timer_stop();
    }
