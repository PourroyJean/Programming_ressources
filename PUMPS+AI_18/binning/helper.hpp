...
enum class Mode : int {
    CPUNormal = 1, GPUNormal, GPUCutoff, GPUBinnedCPUPreprocessing, GPUBinnedGPUPreprocessing
};
...
compute_output(const std::vector<float> &inputValues, const std::vector<float> &inputPositions, int numInputs,
               int gridSize) {
